# 🎯 Bounties

*Special missions with extra rewards*

Bounties are specific tasks that help the project and earn you bonus karma.

---

## 🟢 Active

### 🐛 Bug Hunter · +50 karma
**Difficulty:** Easy

Find and report any bug in the game. Open an [Issue](https://github.com/fabriziosalmi/enjoy/issues/new?template=bug-report.yml) with:
- Clear description of the problem
- Steps to reproduce
- Screenshot if possible

---

### 📚 Documentation Hero · +30 karma
**Difficulty:** Easy

Improve the documentation:
- Fix typos
- Clarify confusing sections
- Add examples

---

### 🔍 Code Reviewer · +75 karma
**Difficulty:** Medium

Leave constructive reviews on 5 open PRs:
- Useful suggestions
- Quality feedback
- Help new contributors

---

### 🌐 Translator · +100 karma
**Difficulty:** Medium

Translate the README to a new language:
- Complete and accurate translation
- Maintain the original tone
- File: `README.{lang}.md`

---

### 🔒 Security Auditor · +200 karma
**Difficulty:** Hard

Find security vulnerabilities:
- Review workflows
- Injection attacks
- Privilege escalation

⚠️ Report responsibly via [Security Advisory](https://github.com/fabriziosalmi/enjoy/security/advisories/new)

---

### 🎨 Theme Creator · +150 karma
**Difficulty:** Medium

Create an alternative theme for the landing page:
- Fork the project
- Create `themes/[name].css`
- Maintain compatibility with existing elements

---

### 🧪 Test Writer · +100 karma
**Difficulty:** Medium

Add automated tests:
- Unit tests for the validator
- Integration tests for workflows
- Edge cases

---

## ⚡ How It Works

1. **Choose** an active bounty
2. **Complete** the required task
3. **Cite** the bounty in your PR/Issue: `Bounty: [name]`
4. **Receive** the bonus karma on merge/close

---

## 🏆 Hall of Fame

*The most prolific bounty hunters*

| Rank | Player | Bounties | Karma |
|------|--------|----------|-------|
| 🥇 | — | — | — |
| 🥈 | — | — | — |
| 🥉 | — | — | — |

---

## 💡 Propose a Bounty

Have an idea for a bounty that helps the project?

[Open an Issue](https://github.com/fabriziosalmi/enjoy/issues/new?labels=bounty-proposal&title=Bounty%20Proposal:%20) with:
- Task description
- Suggested karma
- Why it benefits the project

---

*Bounties are updated weekly.*
